...
USER_AVATAR_SOURCE_DIR="${USER_AVATAR_SOURCE_DIR:-}"
...
USER_AVATAR_SOURCE_DIR="${USER_AVATAR_SOURCE_DIR:-${USER_AVATAR_HOST_DIR:-}}"
...
BASE_NAME="nodeaccess-user-avatars-${TIMESTAMP}"
...
SOURCE_REF="$USER_AVATAR_VOLUME_NAME"

count_archive_entries() {
  tar -tzf "$ARCHIVE_PATH" 2>/dev/null | awk '$0 != "." && $0 != "./" { count++ } END { print count + 0 }'
}

if [[ -n "$USER_AVATAR_SOURCE_DIR" ]]; then
  if [[ ! -d "$USER_AVATAR_SOURCE_DIR" ]]; then
    echo "Diretorio de avatares nao encontrado: $USER_AVATAR_SOURCE_DIR" >&2
    exit 1
  fi

  SOURCE_KIND="directory"
  SOURCE_REF="$USER_AVATAR_SOURCE_DIR"
  echo "[nodeaccess] Gerando backup de avatares a partir do diretorio: $USER_AVATAR_SOURCE_DIR"
  tar -C "$USER_AVATAR_SOURCE_DIR" -czf "$ARCHIVE_PATH" .
else
  if ! command -v docker >/dev/null 2>&1; then
    echo "docker nao encontrado no PATH" >&2
    echo "Informe USER_AVATAR_SOURCE_DIR para backup a partir de diretorio local." >&2
    exit 1
  fi
  ...
  echo "[nodeaccess] Gerando backup de avatares a partir do volume: $USER_AVATAR_VOLUME_NAME"
  docker run --rm \
    -e ARCHIVE_NAME="$(basename "$ARCHIVE_PATH")" \
    -v "${USER_AVATAR_VOLUME_NAME}:/nodeaccess-user-avatars:ro" \
    -v "${OUTPUT_DIR}:/nodeaccess-backups" \
    "$BACKUP_HELPER_IMAGE" \
    sh -lc 'cd /nodeaccess-user-avatars && tar -czf "/nodeaccess-backups/${ARCHIVE_NAME}" .'
fi
...
{
  "type": "nodeaccess-user-avatars-backup",
  ...
  "notes": [
    "This backup contains user avatar files only.",
    "Keep it protected because avatar files may identify users."
  ]
}
EOF

echo "[nodeaccess] Backup de avatares concluido."
