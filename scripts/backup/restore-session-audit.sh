...
SESSION_AUDIT_RESTORE_TARGET_DIR="${SESSION_AUDIT_RESTORE_TARGET_DIR:-}"
...
if [[ -z "$ARCHIVE_FILE" ]]; then
  echo "Uso: bash scripts/backup/restore-session-audit.sh <arquivo.tar.gz> [--yes]" >&2
  exit 1
fi
...
if [[ ! -f "$ARCHIVE_FILE" ]]; then
  echo "Arquivo de auditoria nao encontrado: $ARCHIVE_FILE" >&2
  exit 1
fi
...
if [[ -f "$CHECKSUM_PATH" ]]; then
  ...
  echo "[nodeaccess] Aviso: restore de auditoria sem checksum por RESTORE_REQUIRE_CHECKSUM=false."
fi
...
if [[ -n "$SESSION_AUDIT_RESTORE_TARGET_DIR" ]]; then
  mkdir -p "$SESSION_AUDIT_RESTORE_TARGET_DIR"
  if find "$SESSION_AUDIT_RESTORE_TARGET_DIR" -mindepth 1 -print -quit | grep -q . && [[ "$CONFIRM_FLAG" != "--yes" ]]; then
    echo "Diretorio de destino nao esta vazio: $SESSION_AUDIT_RESTORE_TARGET_DIR" >&2
    echo "Repita o comando com --yes para substituir o conteudo." >&2
    exit 1
  fi

  echo "[nodeaccess] Restaurando auditoria SSH em diretorio: $SESSION_AUDIT_RESTORE_TARGET_DIR"
  if [[ "$CONFIRM_FLAG" == "--yes" ]]; then
    find "$SESSION_AUDIT_RESTORE_TARGET_DIR" -mindepth 1 -exec rm -rf {} +
  fi
  tar -xzf "$ARCHIVE_FILE" -C "$SESSION_AUDIT_RESTORE_TARGET_DIR"
else
  if ! docker volume inspect "$SESSION_AUDIT_VOLUME_NAME" >/dev/null 2>&1; then
    ...
    echo "Informe SESSION_AUDIT_RESTORE_TARGET_DIR para restore em diretorio local." >&2
    exit 1
  fi

  echo "[nodeaccess] Restaurando auditoria SSH no volume: $SESSION_AUDIT_VOLUME_NAME"
  docker run --rm \
    -e ARCHIVE_NAME="$(basename "$ARCHIVE_FILE")" \
    -e CONFIRM_FLAG="$CONFIRM_FLAG" \
    -v "${SESSION_AUDIT_VOLUME_NAME}:/nodeaccess-session-audit" \
    -v "$(dirname "$ARCHIVE_FILE"):/nodeaccess-backups:ro" \
    "$RESTORE_HELPER_IMAGE" \
    sh -lc '
      if find /nodeaccess-session-audit -mindepth 1 -print -quit | grep -q . && [ "$CONFIRM_FLAG" != "--yes" ]; then
        echo "Volume de destino nao esta vazio. Repita com --yes para substituir o conteudo." >&2
        exit 1
      fi
      if [ "$CONFIRM_FLAG" = "--yes" ]; then
        find /nodeaccess-session-audit -mindepth 1 -exec rm -rf {} +
      fi
      tar -xzf "/nodeaccess-backups/${ARCHIVE_NAME}" -C /nodeaccess-session-audit
    '
fi

echo "[nodeaccess] Restore de auditoria SSH concluido."
