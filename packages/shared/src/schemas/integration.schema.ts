...
export const UpsertJiraSchema = z.object({
  ...
  allowedIssueTypes: z.array(z.string().trim().min(1)).max(100).default([]),
  allowedStatuses: z.array(z.string().trim().min(1)).max(100).default([]),
  requiredLabels: z.array(z.string().trim().min(1)).max(100).default([]),
  requireAssignee: z.boolean().default(false),
  maxTicketAgeHours: z.number().int().positive().max(8760).nullable().default(null),
  publishStartComment: z.boolean().default(false),
  publishEndComment: z.boolean().default(false),
  attachAuditOnClose: z.boolean().default(false),
  transitionOnClose: z.boolean().default(false),
  closeTransitionId: z.string().trim().max(100).optional(),
  breakGlassEnabled: z.boolean().default(false),
})
...
export const JiraConfigPublicSchema = z.object({
  ...
  maxTicketAgeHours: z.number().int().positive().nullable(),
  publishStartComment: z.boolean(),
  publishEndComment: z.boolean(),
  attachAuditOnClose: z.boolean(),
  transitionOnClose: z.boolean(),
  closeTransitionId: z.string().nullable(),
  breakGlassEnabled: z.boolean(),
  capabilities: z.object({ read: z.boolean(), comment: z.boolean(), attachment: z.boolean(), transition: z.boolean() }),
  baseUrl:             z.string().nullable(),
