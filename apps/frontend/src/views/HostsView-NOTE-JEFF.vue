...
function normalizeAssociatedLinkOpenMode(raw: unknown): 'new_tab' | 'same_tab' {
  if (typeof raw !== 'string') return 'new_tab'
  const normalized = raw.trim().toLowerCase()
  if (normalized === 'same_tab' || normalized === 'sametab') return 'same_tab'
  return 'new_tab'
}

function normalizeAssociatedLinkSourceType(raw: unknown): 'manual' | 'integration' | 'derived' {
...
function normalizeAssociatedLinkSourceStatus(raw: unknown): 'manual' | 'synced' | 'stale' | 'error' {
  if (typeof raw !== 'string') return 'manual'
  const normalized = raw.trim().toLowerCase()
...
function normalizeAssociatedLinks(links: HostAssociatedLinkForm[] | undefined): HostAssociatedLink[] {
  return (links ?? [])
    .map((link, position) => ({
      ...
      sourceType: normalizeAssociatedLinkSourceType(link.sourceType),
      sourceStatus: normalizeAssociatedLinkSourceStatus(link.sourceStatus),
      position,
    }))
...
type AgentIndicator = { color: 'green' | 'orange'; tooltip: string } | null

function agentIndicatorForHost(host: HostPublic): AgentIndicator {
  const mode = host.connectionMode
  if (mode === 'direct') return null
  const st = agentStatus.value

  if (mode === 'agent') {
    // Precisa obrigatoriamente de um SERVICE_BOUND tenant agent
    const a = st?.tenantAgent ?? st?.userAgent
    if (a) return { color: 'green',  tooltip: t('hosts.agent.online',   { name: a.name }) }
    return        { color: 'orange', tooltip: t('hosts.agent.required') }
  }

  if (mode === 'agent_user') {
    const a = st?.userAgent
    if (a) return { color: 'green',  tooltip: t('hosts.agent.online',   { name: a.name }) }
    return        { color: 'orange', tooltip: t('hosts.agent.required') }
  }

  // agent_tenant_fallback / auto — funciona mesmo sem agente (fallback direto)
  const a = st?.userAgent ?? st?.tenantAgent
  if (a) return { color: 'green', tooltip: t('hosts.agent.online', { name: a.name }) }
  return null // sem agente, mas conexão ainda funciona via direct
}
  ...
  <div class="flex h-screen overflow-hidden">
    ...
    <div class="flex-1 overflow-auto p-6" @contextmenu="onHostAreaContextMenu">
      ...
      <NSpin :show="loading">
        ...
        >
          <div class="hidden lg:grid grid-cols-[minmax(180px,1.4fr)_110px_130px_minmax(120px,0.8fr)_110px_minmax(140px,0.8fr)_220px] gap-3 border-b border-gray-800 px-4 py-3 text-[11px] font-semibold uppercase tracking-wider text-gray-500">
          ...
          <div
            ...
            @dragend="onDragEnd"
          >
            <div class="hidden lg:grid lg:grid-cols-[minmax(180px,1.4fr)_110px_130px_minmax(120px,0.8fr)_110px_minmax(140px,0.8fr)_220px] gap-3 items-center">
              ...
              <div class="text-xs text-gray-300">
                ...
                <div class="mt-1 flex items-center gap-1.5">
                  <span class="text-[11px] text-gray-500">{{ connectionModeLabel(host.connectionMode) }}</span>
                  <NTooltip v-if="agentIndicatorForHost(host)" trigger="hover" placement="top">
                    <template #trigger>
                      <span
                        class="w-1.5 h-1.5 rounded-full shrink-0"
                        :class="agentIndicatorForHost(host)!.color === 'green' ? 'bg-green-400' : 'bg-orange-500'"
                      />
                    </template>
                    {{ agentIndicatorForHost(host)!.tooltip }}
                  </NTooltip>
                </div>
                <NTooltip>
                  <template #trigger>
              ...
              <div class="min-w-0">
                <div v-if="visibleAssociatedLinks(host).length" class="flex min-w-0 flex-wrap gap-1.5 overflow-hidden">
                  <template v-for="link in visibleAssociatedLinks(host)" :key="`list-link-${host.id}-${link.label}-${link.position}`">
                    <NTooltip>
                      <template #trigger>
            ...
            <div class="lg:hidden">
              ...
              <div class="mt-2 text-xs text-gray-300">
                ...
                <NTooltip v-if="agentIndicatorForHost(host)" trigger="hover" placement="top">
                  <template #trigger>
                    <span
                      class="inline-block w-1.5 h-1.5 rounded-full align-middle ml-1"
                      :class="agentIndicatorForHost(host)!.color === 'green' ? 'bg-green-400' : 'bg-orange-500'"
                    />
                  </template>
                  {{ agentIndicatorForHost(host)!.tooltip }}
          ...
          <NGridItem v-for="host in filteredHosts" :key="host.id">
            ...
            >
              ...
              <div v-if="visibleAssociatedLinks(host).length" class="mt-2 flex flex-wrap gap-1.5">
                ...
                <NTag v-if="hiddenAssociatedLinkCount(host) > 0" size="small">
