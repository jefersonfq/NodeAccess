...
export class AgentGateway {
  ...
  async handleConnection(ws: WebSocket, rawToken: string, meta: AgentConnectionMeta = {}): Promise<void> {
    ...
    void this.agentService.logConnected(agent.id, agent.name, agent.agentMode, agent.createdById)
    ...
    ws.on('close', (code: number) => {
      ...
      void this.agentService.logDisconnected(agent.id, agent.name, agent.agentMode, agent.createdById, disconnectReason)
    })
  }
}
