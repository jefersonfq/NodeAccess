...
export async function agentRoutes(app: FastifyInstance, ctrl: AgentController): Promise<void> {
  ...
  app.get('/status', {
    ...
    handler: async (req, reply) => {
      const { sub, tenantId } = (req as FastifyRequest & { user: { sub: string; tenantId: number } }).user
      const u = agentRegistry.getForUser(Number(sub))
      const t = agentRegistry.getForTenant(tenantId)
      return reply.send({
        userAgent:   u ? { id: u.agentId, name: u.name } : null,
        tenantAgent: t ? { id: t.agentId, name: t.name } : null,
      })
    },
  })
  ...
  app.get('/install/linux', {
    schema: { tags: tag, summary: 'Script de instalação para Linux (bash)' },
...
`
  ...
  app.get('/install/macos', {
    schema: { tags: tag, summary: 'Script de instalação para macOS (bash)' },
    handler: async (_req, reply) => {
      const s = env.APP_URL
      const script = `#!/usr/bin/env bash
set -euo pipefail
...
`
  ...
  app.get('/install/windows', {
    schema: { tags: tag, summary: 'Script de instalação para Windows (PowerShell)' },
    handler: async (_req, reply) => {
      const s = env.APP_URL
      const script = `param(
