...
import { hostImportRoutes } from './host-import.routes'
import { HostImportController } from './host-import.controller'
...
async function appFor(role: 'admin' | 'user' = 'admin', canManageHosts = true) {
  const service = {
    preview: vi.fn(async () => ({
      ...
      summary: { detected: 1, ready: 1, blocked: 0, foldersToCreate: 1, aclMappings: 0, warnings: 0 },
      report: [{ sourceId: 'source-1', name: 'Linux', status: 'ready', destinationPath: 'Inventário / Produção', warnings: [] }],
    })),
    commit: vi.fn(async () => ({
      status: 'committed', createdHosts: 1, createdFolders: 1, appliedAclMappings: 0,
      rolledBackHosts: 0, rolledBackFolders: 0,
  ...
  await app.register(async instance => hostImportRoutes(instance, new HostImportController(service as never)))
  return { app, service }
}
...
describe('host import HTTP routes', () => {
  it('validates preview input and forwards active tenant/user context', async () => {
    ...
    try {
      ...
      expect(response.json().summary.ready).toBe(1)
