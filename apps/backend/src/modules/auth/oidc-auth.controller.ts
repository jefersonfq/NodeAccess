...
export class OidcAuthController {
  ...
  async begin(request: FastifyRequest<{ Body: { tenantSlug?: string } }>, reply: FastifyReply) {
    ...
    await this.rateLimit.check({ action: 'oidc_begin', ip: request.ip, tenant: slug })
    return reply.send(await this.service.begin(slug))
  }

  async complete(request: FastifyRequest<{ Body: { state: string; code: string } }>, reply: FastifyReply) {
    await this.rateLimit.check({ action: 'oidc_complete', ip: request.ip, identity: request.body.state })
